...
--peer-consolidate is accepted as an alias for --smart-consolidate, for
symmetry with mds2brocade. Either spelling works.`,
	Args: cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		outputFile, _ := cmd.Flags().GetString("output")
		// --peer-consolidate is a hidden alias of --smart-consolidate so users
		// who learned the flag name from mds2brocade get the same behavior here.
		smartConsolidate, _ := cmd.Flags().GetBool("smart-consolidate")
		peerConsolidate, _ := cmd.Flags().GetBool("peer-consolidate")
		consolidate := smartConsolidate || peerConsolidate
...
func init() {
	...
	// Hidden alias of --smart-consolidate; documented in USER_GUIDE.md.
	brocade2mdsCmd.Flags().Bool("peer-consolidate", false, "alias of --smart-consolidate (for symmetry with mds2brocade)")
	_ = brocade2mdsCmd.Flags().MarkHidden("peer-consolidate")
	brocade2mdsCmd.Flags().String("consolidate-report", "", "write the consolidation report to this file")
	brocade2mdsCmd.Flags().Bool("consolidate-strict", false, "with --smart-consolidate/--peer-consolidate: require an exact <host>_<target> zone name (default: also consolidate when the target alias is a trailing component of the zone name)")
}
